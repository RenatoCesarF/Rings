...
class Glow():
    def __init__(self, width: int = 10, height: int = 10, color: tuple = (0,0,0),radius: int = 0):
        self.radius = radius
        self.color = color
        self.width = width
        self.height = height
    ...
    def circle_glow(self) -> pygame.Surface:
        surf = pygame.Surface((self.width*2,self.height*2))#self.radius*2, self.radius*2))
        ...
        pygame.draw.rect(surf, self.color, 
                        ...
                        width = 0,border_radius = self.radius)
