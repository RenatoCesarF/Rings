...
class  Collider:
    def __init__(self, initial_position: Vector, width: int, height: int, collision_tolerance: int = 3) -> None:
    ...
    # def is_coliding_with(self,other_collider: Collider):
    ...
    def check_colliding_with(self, other_collider: Collider) -> None:
        ...
        self.__check_sides_collisition(other_collider) #TODO: check colision at side, and then at top and bottom as potatto does
    ...
    def update_position_with_current_speed(self):
        self.collision_rect.x += self.speed.x
        self.collision_rect.y += self.speed.y  

    def draw_collision_rect(self, destination_surface: pygame.Surface, offset: Vector = Vector(), color: tuple = (250,0,0), width: int = 1) -> None:
        pygame.draw.rect(destination_surface, color, 
                         pygame.Rect(self.collision_rect.x - offset.x,
                                     self.collision_rect.y - offset.y,
                                     self.width,self.height), width=width)

    def set_all_collisiont_to_false(self) -> None:
        self.is_colliding = False
        self.is_colliding_top = False
        self.is_colliding_bottom = False
        self.is_colliding_left = False
